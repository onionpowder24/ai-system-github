...
namespace Aituber
{
    public class SubtitleManager : MonoBehaviour
    {
        ...
        {
            Debug.Log("HandleInputFieldEnter called with input: " + userInput);
            // QueueManagerに質問を送る
            if (queueManager != null && !string.IsNullOrEmpty(userInput))
            {
                Debug.Log("Creating question and adding to queue...");
                var question = new Question(userInput, "ユーザー", "Sprites/usericon_noname", false);
                queueManager.AddTextToQueue(question);
            }
            else
            {
                Debug.LogWarning("QueueManager is null or userInput is empty! QueueManager: " + queueManager + ", Input: " + userInput);
            }
        ...
        void Start()
        {
            answerInputField.onSubmit.AddListener(HandleInputFieldEnter);
        }
    }
}